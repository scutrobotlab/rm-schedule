...
- **当前比赛竞猜预测**：`/api/current_match_forecast` 从实时 `schedule` 缓存里找 `status==STARTED` 的比赛（约定同一时刻只有一场，取第一场即可），下发赛区名/场次号/slug、红蓝双方校徽·校名·队名，并复用 `/mp/match` 支持率；`support_rate_deadline` 为该 match 支持率从 `mp.robomaster.com` 查询的时刻（精确到秒，东八区）；无进行中比赛时 `has_match=false`
